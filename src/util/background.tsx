...
const Background = () => {
    const cx = `${getRandInt()}%`;
    const cy = `${getRandInt({ max: 30 })}%`;
    const rx = `${getRandInt({ min: 500, max: 600 })}px`;
    const ry = `${getRandInt({ min: 200, max: 300 })}px`;
    const rotate = `rotate(${getRandInt({ min: -30, max: 30 })})`;

    return (
        <svg xmlns="http://www.w3.org/2000/svg" style={style}>
            <ellipse cx={cx} cy={cy} rx={rx} ry={ry} fill="rgba(0, 0, 255, 0.1)" filter="blur(2rem)" transform={rotate} />
        </svg>
    );
};
